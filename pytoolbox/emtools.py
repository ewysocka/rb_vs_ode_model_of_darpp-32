...
def list2file(file, list_of_lists):
    ## fiven list has to be a list of list
    ## if not, use function 'unflatten'

    with open(file, 'wt') as file_out:
         listwriter=csv.writer(file_out)
...
def list2file_TAB(file, list_of_lists):
     with open(file, 'wt') as file_out:
         listwriter=csv.writer(file_out, delimiter='\t')
...
def file2list(file):
    #####################################################
    #Requirements: file must be csv.

    #dialect = csv.Sniffer().sniff(open(file).read())
    with open(file, 'rt') as file_in:
        listreader=csv.reader(file_in) #, dialect=dialect)
        lista=[row for row in listreader]

    # for single column otherwise it returns a list of lists
  #
  # lista_ = [l[0].split(';') for l in lista]

    return lista



def flatten_listoflists(input):
    return [j if isinstance(item, list) else item for item in input for j in item ]


def unflatten_listoflists(input):
    return [[item] if isinstance(item, str) else item for item in input ]


def get_uniq(filein, fileout):
    #####################################################
    # Returns a unique list of one column  list

    lista = file2list(filein)
    lista = flatten_listoflists(lista)
    unique_lista = list(set(lista))

    unique_lista2= unflatten_listoflists(unique_lista)
    list2file(fileout,unique_lista2)
    return len(unique_lista2)



def get_uniq2(listin, column_no):
    #####################################################
    # Returns a unique list of indicated column
    listout = [i[column_no] for i in listin]
    return list(set(listout))


def intersect(lista1, lista2):
    ########################################
    # Give an intersection of two lists
...
# merged.drop(filter.index, inplace=True)




def get_symb(genelist):
    #######################################
    # GET symbols for Entrez gene list (for network labels)

    conn = msdb.connect(host="nrg.inf.ed.ac.uk", user="emilia", passwd="kappa", db="emilia")
    cur = conn.cursor()

    ent2sym=[]

    for gene in genelist:
        cur.execute("select * from emilia.hmr_entrez2gene where tax_id=9606 and entrez_id=%s",[gene])
        print('Finished execution')
        a = cur.fetchall()
        print('Everything is fetched')
        ent2sym.append(a)

    ent2sym3 = [j[1] for i in ent2sym for j in i]   # TO CO TRZEBA :  LIST OF  symbols
    return ent2sym3


### - to csv as required by the topGO specification of annotation files:
def uniprotTOstr(lista):
    # UniProt Accessions to string:
    return ', '.join(lista)



def to_topGOannotation(dict_of_lists):
    # save in the form:  'other_id', 'uniprotID01, uniprotID02, ... '

    annot = []
    for key, value in dict_of_lists.iteritems():
        uniprot = uniprotTOstr(value)
        annot.append([key, uniprot])
...
#     df_morethan = df[df[col1] > df[col2]].index

#     df.loc[df_morethan, col1] = df[col2]
#     df.loc[df_morethan, col2] = df[col1]
